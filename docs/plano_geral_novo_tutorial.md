...
| | **5 blocos de conteúdo (FRAMING CORRETO validado 2026-04-12):** (1) O que o NOVO CR faz — filosofia geral: relatórios detalhados, dados sobre lacunas, (2) Como funciona — IA compara gabarito vs resposta (classificação), professor mantém controle, (3) Por que funciona tão bem — classificação pura, critérios aumentam precisão, (4) Case real — FGV: monitoria, filas, pontos extras; NOVO CR automatiza devolutivas, (5) Para quem serve — aluno entende erros, professor vê padrões, instituição tem dados longitudinais. **NÃO é 'problema vs solução'. É 'filosofia geral vs case específico'.** | |

...
oficiais.
**Runtime oficial atual:** backend Render em `deb1e2a`; `origin/main` recebeu os
...
legiveis).
`deb1e2a` fecha a lacuna de custos row-level em etapas multimodais simples:
depois do deploy, `EXTRAIR_QUESTOES` com `gem25flash001` criou
`2335d8b186105ab5` e `usage_498f405580df4408`, elevando
`token_usage.record_count` para `9`.
O
...
- `/api/deploy-info` confirmou o runtime backend `52ff747` com
  `source=RENDER_GIT_COMMIT`; esse e o gate primario atual para codigo live.
- `origin/main` pode ficar a frente do runtime em commits documentais; para
  codigo backend, o gate de comportamento provider/pipeline continua sendo
  `/api/deploy-info` com no-cache e smokes live, nao apenas commit local.
- `/api/custos/status?limit=100` em `c8f538a` retornou `ok=true`,
  ...
  `token_usage_backend.durable=true`, mas ainda com `token_usage_analisados=0`.
  Esse bloqueio foi fechado depois: smokes em `518f8a2` e `58781a1` elevaram
  `record_count` para `2`, `token_usage_analisados=2` e `alertas=[]`.
...
- Deploy `58781a1` confirmou a preferencia por PDF em agregados:
  `desempenho_materia` Matemática-V `run-20260519-120054` ficou `PARCIAL` por
  apenas um dado ausente real (Erik/Omega sem `RELATORIO_FINAL`), sem os avisos
  antigos de Daniel/Beta-V; custo `28889/3299`, `US$0.016914`, usage
  `usage_c53952166c3d40ce`, `record_count=2`, `token_usage_analisados=2`.
- Deploy `f534576` corrigiu falso verde por `max_iterations_exceeded`. O smoke
  Haiku anterior (`run-20260519-121133`) tinha retornado `COMPLETO` com limite
...
- Deploy `52ff747` corrigiu a leitura desses dois smokes: antes,
  `/api/desempenho/tarefa/810ef4c1a71c701b` agrupava os quatro docs em um run por
  proximidade temporal; depois, o readback mostra `run-tool_ae40e3a59695` e
  `run-tool_922168f5c256` separados por `cost_run_id`. Validacoes locais:
  `py_compile`, `git diff --check`, `test_desempenho_api_endpoints.py` e
  `test_desempenho_no_duplicate_save.py` com `26 passed`. O check oficial foi
  GET/readback, sem nova chamada de IA.
...
- Full pipeline o3-low: `task_f200c8d9abf4` completou as 6 etapas com
  `58ff5dcdff67` na fixture Diana, sem `stage_errors`; custo medido
...
- Pipeline live GPT-5 Nano: `task_0818b99194aa` passou `extrair_respostas` na
  fixture Diana; documento `f021525b6fdf0db0`, respostas reais `x = 5`, `34`,
  `25`, `20 cm2`, custo `US$ 0.000914`. Continuar parcial fora dessa fixture.
- Pipeline live GPT-5 Nano: `task_960c0a287a13` passou `corrigir` na fixture
...
- Nano full task pos-`dbbecfe`: `task_4f6296b3789d` falhou alto em `corrigir`
  porque o JSON final tinha `feedback_geral`, mas o PDF final
  `fb0825ca9ccbec85` nao tinha esse texto verificavel. O erro ficou visivel e
  os PDFs intermediarios permaneceram `status=erro`.
- Nano full task pos-`4a4caf0`: `task_cbe8568e78d6` completou as seis etapas
  com `gpt5nano001`, sem `stage_errors`. Artefatos principais: questoes
  `8afc777cde9edfeb`, gabarito `01c96fa3a0d69b19`, respostas
  `31d3a713877dce09`, correcao JSON/PDF `728fcc2e2722c486` /
  `c545aab82c27a698`, habilidades JSON/PDF `419ef6b546babaa6` /
...
| `4eaeb5105f5d` | Claude Sonnet 4.5 | `anthropic/claude-sonnet-4-5-20250929` | T/V | `3.00/15.00` | oficial Anthropic | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ⏸️ nao testado | Falha historica `task_b19524abfdd5` por JSON em Markdown foi superada para `EXTRAIR_QUESTOES`: `task_b9eb22dbb753`, doc `147b412840c2b618`, `2257/461`, `US$0.013686`. Pos-`deb1e2a`, `EXTRAIR_GABARITO` passou em `task_f6f7260d5a77`, doc `ce57fd9cc1cbbdac`, `7731/1585`, `US$0.046968`, usage `usage_7a50263d5d16434c`. `EXTRAIR_RESPOSTAS` passou em `task_70ecb29b3434`, doc `6f98a33dd2f98770`, `3719/708`, `US$0.021777`, usage `usage_9d683b3f19754814`, 4 respostas reais, 0 branco/ilegivel. `CORRIGIR` passou em `task_baf5c638a3d8`, JSON `0556518140b3222c`, PDF `f993aabbf6835583`, `16405/5308`, `US$0.128835`, usage `usage_fe8d1854ee36466e`, nota 8.0 e `Feedback Geral` no PDF. `ANALISAR_HABILIDADES` passou em `task_285f0f62a5cd`, JSON `678d08cdef1e323e`, PDF `777aea7f9d12f677`, `29462/8315`, `US$0.213111`, usage `usage_8e51a29148fc4b84`, proficiencia 0.8. `GERAR_RELATORIO` passou em `task_a0dac5f0e565`, JSON `f48b635c52d030b7`, PDF `72d63fecc800cf5a`, `18561/6493`, `US$0.153078`, usage `usage_5bf3982306644110`, relatorio legivel com nota 8.0 e fontes Corr/Hab, `record_count=14`. | `US$ 0.408816` | Proximo Sonnet: full pipeline em task unica se o objetivo justificar custo; etapas isoladas ja estao OK. |
| `gem25flash001` | Gemini 2.5 Flash | `google/gemini-2.5-flash` | T/V | `0.30/2.50` | oficial Google | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ Beatriz + agregados Matemática-V | Pipeline individual `task_ca5dd6b8b3b5`: seis etapas OK, `117829/31691`, `US$0.114578`; agregados: tarefa `run-20260519-112430` completa `15858/3404`, `US$0.013267`; turma `run-20260519-112612` completa `30310/9049`, `US$0.031716`; materia pos-`58781a1` `run-20260519-120054` parcial honesta por Erik/Omega sem relatório, `28889/3299`, `US$0.016914`, usage `usage_c53952166c3d40ce`; tarefa pos-`e85be11` limpa sem `.md` fantasma: `usage_459e3a56a73748fc`, `16939/3300`, `US$0.013332`, docs `afa143d8e6390caf`/`692d50f8be3d885d`; readback pos-`52ff747` separa esse run do `usage_ac21f90610244c4b` por `cost_run_id`; Omega-V pos-`546b72f` mantém 2 docs válidos em tarefa e turma e eleva row-level para `record_count=8`; pos-`deb1e2a`, `EXTRAIR_QUESTOES` criou doc `2335d8b186105ab5` e usage `usage_498f405580df4408`, `3385/1058`, `US$0.003661`, `record_count=9`. | `US$ 0.053285` | Proximo: completar `RELATORIO_FINAL` do Erik/Omega so com dado real e repetir matéria; contrato de artefato extra em tarefa/turma e custo row-level multimodal ja estao provados. |
...
| `e251747cd7a2` | Gemini 2.5 Pro | `google/gemini-2.5-pro` | T/V | `1.25/10.00` ate 200k prompt | oficial Google | ⏸️ | ⏸️ | ⏸️ | ⏸️ | ⏸️ | ⏸️ | ⏸️ nao testado | Sweep live 2026-05-19: conexao OK, `197` tokens; pipeline ainda nao testada por custo. | `US$ 0.216851` | Testar uma etapa barata somente se houver objetivo claro para Pro. |
...
- Recheck 2026-05-19 do endpoint live de custo confirmou:
  ...
  o4 Mini `US$0.136256`.
- Sweep de conexao 2026-05-19 via `/api/settings/models/{id}/testar`: 13/14
...
## Matriz Anthropic -- Relatorios De Desempenho Agregado

| Modelo Anthropic | `desempenho_tarefa` | `desempenho_turma` | `desempenho_materia` | Evidencia | Proximo passo |
|---|:---:|:---:|:---:|---|---|
| `588f3efe7975` Haiku 4.5 | ⚠️ | ⏸️ | ⏸️ | `run-20260519-122041` passou tarefa Alpha Algebra, 2 alunos/0 excluidos, `151975/26024`, `US$0.282095`; ressalva: custo/latencia altos e run anterior bateu `max_iterations_exceeded`. | Nao subir para turma/materia sem decisao explicita de comparar qualidade contra Google Flash. |
| `4eaeb5105f5d` Sonnet 4.5 | ⏸️ | ⏸️ | ⏸️ | Nao testado em agregado; Sonnet ja falhou em extração e e mais caro. | Pausar ate Haiku/Google cobrirem o produto principal. |
...
(`task_cbe8568e78d6`), com artefatos/custos rastreados e PDF de correcao
contendo `Feedback Geral` verificavel contra o JSON. Isso nao promove Nano a
pipeline-ready geral: ainda falta rodar dataset maior e casos menos simples.

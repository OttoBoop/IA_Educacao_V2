...
4. Os fixes principais ja chegaram ao site oficial ate `1ce3d23`: Render MCP
   confirma o servico `srv-d5t8gbh4tr6s738fr3s0` com deploy live
   `dep-d841f437uimc73fs60lg`, enquanto o HTML marker segue atrasado em
...
9. Gemini 3 Flash esta confirmado em 5 etapas individuais do aluno, mas
   ...
   candidato explicito para `extrair_respostas` e completou a mesma amostra com
   4/7 respostas reais, 3/7 `MISSING_CONTENT`, tokens `97004/1942` e custo
   `US$ 0.081492`; isso valida a etapa nessa amostra, nao pipeline completa.
   Haiku segue bloqueado por creditos; Rio 3 esta pausado.
...
| Git/GitHub | Render MCP confirmou runtime `1ce3d23`; `origin/main` contem os commits `8dd6c54`, `c1598b9`, `01fb04c`, `6b57ef1`, `3b9eedc`, `b8b8693`, `283e8c6` e `1ce3d23` da sequencia anti-respostas-vazias/inferidas | Nao usar somente marker HTML como gate quando Render `rootDir=backend` ignora commits sem backend; combinar MCP/deploy list + comportamento live. |
...
| Tokens/custos | Split input/output; metadata de documento; endpoints `/api/custos/status` e `/api/custos/resumo` respondendo live; resumo agrega por `cost_run_id`; `TokenUsageRecord` local cobre falha sem documento enquanto o filesystem vive; codigo Supabase e migration dedicada `b2dc88b` existem; diagnostico live mostra `PGRST205`; depois da falha alta `task_3d5feaf0da71`, `usage_52590d55d210459e` registrou tokens `100188/8863`, custo `US$ 0.008555`, mas deploy posterior zerou o store local; GPT-5.4 Mini registrou docs `a39d26fcc621c7a8`, `fec100a2e41eabcf` e `4a82ddf1d2118ff0`, custos `US$ 0.081492`, `US$ 0.080570` e `US$ 0.0806`; `durable=false` | Falta aplicar `backend/migrations/002_create_token_usage.sql` no Supabase. |
| Providers | Gemini passou em chat simples live, `extrair_questoes`, `extrair_respostas` e nas tres etapas finais; `extrair_gabarito` Gemini foi reclassificado como invalido por tudo `MISSING_CONTENT`; GPT-5 Nano passou em chat simples live, `extrair_questoes`, `extrair_gabarito` pos-`5527e26` e nas tres etapas finais; `extrair_respostas` Nano foi reclassificado como invalido por tudo `ilegivel`/vazio, por inferencia suspeita ou por scan majoritariamente sem conteudo, e agora falha alto desde `1ce3d23`; GPT-5.4 Mini passou `extrair_respostas` em uma amostra efemera e duas amostras com `gpt54mini001` versionado; runner destacado e bloqueio de rotas legadas ja tiveram comportamento observado no backend live | Validar GPT-5.4 Mini em pipeline per-phase; rerodar `extrair_gabarito` Gemini quando quota/decisao permitir. |
...
| Git/deploy | Commit `2d72c6b` adicionou `/api/deploy-info` e fez `check_deploy.sh` priorizar o commit reportado pelo backend; Render `dep-d84bjopo3t8c73fbshug` esta live e `/api/deploy-info` retorna `source=RENDER_GIT_COMMIT` | Usar esse gate antes de novos smokes; marker HTML e apenas auxiliar. |

### Bloqueios E Alertas

| Item | Estado | Acao correta |
|---|---|---|
| Render/site oficial | Render MCP lista `1ce3d23` como deploy live; HTML marker ainda aponta `e6060e1` | Tratar HTML marker como auxiliar; corrigir verificador em ciclo proprio. |
...
1. Corrigir a causa de `extrair_respostas` sem conteudo real no GPT-5 Nano
   (prompt, leitura de PDF/imagem, modelo com OCR/vision melhor ou decisao
   explicita de inadequacao), mantendo a falha alta de `1ce3d23` como protecao.
...
8. Retomar Rio 3 apenas por decisao explicita.
...
O estado oficial agora e melhor que estava no inicio desta auditoria: Render MCP
confirmou `1ce3d23` live. A palavra "feito" continua exigindo quatro provas:
...
falha registrado em `usage_52590d55d210459e`; Haiku esta bloqueado por creditos
...
- Corrigir `extrair_respostas` para extrair conteudo real; `1ce3d23` ja impede
  falso sucesso final, mas nao resolve a qualidade da extracao.
- Corrigir o gate de deploy/verificador: Render MCP confirma `1ce3d23`, mas o
  HTML marker segue atrasado em `e6060e1`.
...
- Revalidar providers depois dos fixes atuais: Gemini `extrair_gabarito`, Nano
  `extrair_respostas` corrigido, Haiku quando houver credito.
...
|---|---|---|---|
| Gate oficial confuso | O HTML marker continua sendo stale por `rootDir=backend`; o commit `2d72c6b` criou `/api/deploy-info` para o backend reportar o runtime. | Render MCP confirmou `dep-d84bjopo3t8c73fbshug` live; `/api/deploy-info` retornou `2d72c6b` via `RENDER_GIT_COMMIT`; `check_deploy.sh 2d72c6b` passou. | Usar `/api/deploy-info` como gate primario. |
...
| Provider matrix ainda incompleta | Doc 12 foi atualizado com `1ce3d23`, mas nem todos os providers/rotas estao confirmados. | Gemini `extrair_gabarito` invalido; Nano `extrair_respostas` falha alto mas nao extrai; Haiku bloqueado por credito. | Smoke por provider/rota depois de cada fix real, com custo e erro registrados. |
...
| GPT-5 Nano | Chat OK; `extrair_questoes`, `extrair_gabarito` e `corrigir` OK no smoke integrado pos-`f2211bb`; `analisar_habilidades` tem sucesso individual historico, mas falhou alto no run integrado; `extrair_respostas` falhou conteudo e agora falha alto desde `1ce3d23`; falta pipeline completa | Smoke integrado `task_19ee59ac1881`: `extrair_questoes` `d50f3b909e6773e7` (`US$ 0.003580`), `extrair_gabarito` `8dd414ee1617c3a5` (`US$ 0.002545`), `corrigir` JSON/PDF `f0302debf41ae58f`/`31794fc784905c00` (`US$ 0.002807`), falha em `analisar_habilidades` com doc erro `b5f17f2d1a980a3d` (`US$ 0.004213`). Historico: `extrair_respostas` falhou em `task_3d5feaf0da71`; analise individual ja passou em `task_020ba25bdb2b`; relatorio individual ja passou em `task_aec830b85c03`. | Corrigir `analisar_habilidades` tool-use no run integrado; manter Nano fora de `extrair_respostas`; depois validar schema minimo/pipeline completa. |
| GPT-5.4 Mini | `extrair_respostas` OK em duas amostras avulsas e no smoke integrado pos-`f2211bb`; demais etapas nao testadas | Smoke usou cadastro efemero `04b31001cf81`; teste de conexao retornou `OK`; `task_9c10e3752bcb` criou JSON `a39d26fcc621c7a8`, 4/7 respostas com conteudo real, 3/7 `MISSING_CONTENT`, tokens `97004/1942`, custo `US$ 0.081492`; depois `gpt54mini001` entrou em `backend/data/models.json`; `task_706931a94555` criou `fec100a2e41eabcf`, tokens `97004/1737`, custo `US$ 0.080570`; `task_19062336eb8b` criou `4a82ddf1d2118ff0`, 7/7 respostas reais, tokens `90588/2813`, custo `US$ 0.0806`; no smoke integrado `task_19ee59ac1881`, `gpt54mini001` criou `1e5db36f3ab9aa0e`, tokens `18176/2081`, custo `US$ 0.022996`. | Manter como modelo explicito para `extrair_respostas`; testar outras fases somente quando a tese do ciclo pedir. |
...
- GPT-5.4 Mini: `extrair_respostas` passou em amostras oficiais; settings
  ...
  `task_19ee59ac1881`, mas a pipeline completa ainda falhou depois, em

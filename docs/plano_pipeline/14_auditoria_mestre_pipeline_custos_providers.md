...
confirmado no site oficial e `e2260d2`, validado por `/api/deploy-info`,
`/api/health`, `check_deploy.sh`, smoke live de `/api/custos/status` e smoke do
HTML oficial. Smokes anteriores continuam validando documentos/ranking/dashboard
nos hashes registrados em seus proprios ciclos. O ciclo `147296d` tambem reduziu o
...
no alerta de custos nao duraveis. Commits
...
| Git/deploy | Commit `2d72c6b` adicionou `/api/deploy-info`; o codigo funcional mais recente confirmado e `e2260d2` | Usar `/api/deploy-info` antes de novos smokes; marker HTML e apenas auxiliar; commits documentais podem mudar o hash sem mudar comportamento. |

### Bloqueios E Alertas

| Item | Estado | Acao correta |
|---|---|---|
| Render/site oficial | `/api/deploy-info` confirmou `e2260d2` como deploy live no patch que torna o bloqueio de migration `token_usage` visivel no dashboard; ciclos anteriores confirmaram resultado parcial/historico/status obedecendo `status=erro`, itens avaliaveis, rotas agregadas protegidas, media zero, agregados em lote, default GPT-5.4 Mini e erro de provider resumido | Tratar HTML marker como auxiliar; usar smoke real para aceite; se doc-only posterior mudar hash, preservar `e2260d2` como ultimo codigo funcional validado. |

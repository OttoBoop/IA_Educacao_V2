...
9. Gemini 3 Flash esta confirmado em 5 etapas individuais do aluno, mas
   ...
   PDF a partir do JSON validado. Haiku segue bloqueado por creditos; Rio 3
   esta pausado.
...
1. Deployar/revalidar o retry PDF/JSON: quando PDF de `CORRIGIR` ou
   `GERAR_RELATORIO` divergir do JSON, regenerar apenas o PDF no mesmo modelo;
   se continuar divergente, falhar alto.
...
1. Deployar/revalidar o retry PDF/JSON descoberto a partir de
   `task_857c0c3657ef`.

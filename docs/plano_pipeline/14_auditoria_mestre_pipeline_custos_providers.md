...
confirmado no site oficial e `48407f2`, validado por `/api/deploy-info`,
...
| Git/deploy | Commit `2d72c6b` adicionou `/api/deploy-info`; o codigo funcional mais recente confirmado e `48407f2` | Usar `/api/deploy-info` antes de novos smokes; marker HTML e apenas auxiliar; commits documentais podem mudar o hash sem mudar comportamento. |

### Bloqueios E Alertas

| Item | Estado | Acao correta |
|---|---|---|
| Render/site oficial | `/api/deploy-info` confirmou `48407f2` como deploy live no patch de resultado parcial/historico/status obedecendo `status=erro`, itens avaliaveis, rotas agregadas protegidas, media zero, agregados em lote, default GPT-5.4 Mini e erro de provider resumido | Tratar HTML marker como auxiliar; usar smoke real para aceite; se doc-only posterior mudar hash, preservar `48407f2` como ultimo codigo funcional validado. |

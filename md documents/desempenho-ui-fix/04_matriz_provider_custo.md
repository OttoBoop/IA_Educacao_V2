...
## ⚠️ Caveat crítico sobre o run Anthropic

O PDF do gabarito do professor para Lista0 contém **apenas a resolução da Q5**. As questões 1, 2, 3, 4, 6 e 7 não têm gabarito disponível. Tanto Anthropic (Claude Haiku 4.5) quanto Gemini 3 Flash extraem corretamente: gabarito tem `_avisos_questao: MISSING_CONTENT` pras 6 questões.

O backend ([executor.py:2190-2198](../../backend/executor.py#L2190)) tem um guard que **aborta CORRIGIR** quando há `MISSING_CONTENT` no gabarito. Anthropic conseguiu passar porque **ALUCINOU gabarito completo em retries** (chamou `extracao_gabarito` 91 vezes; algum retry preencheu Q1-Q4/Q6-Q7 com texto inventado, e o guard deixou passar). Gemini foi honesto e bateu no guard imediatamente.

**Auditoria** ([_audit_20260525_214456/audit.json](./_audit_20260525_214456/audit.json)) confirma: **10/10 relatórios Anthropic têm correção baseada em gabarito alucinado em 6 das 7 questões**. As notas finais não têm valor científico.

...
Se não houver critérios parciais explícitos, deixe `criterios_parciais` como lista vazia `[]`."""
    ...
    EtapaProcessamento.EXTRAIR_RESPOSTAS: PromptTemplate(
        ...
        texto_sistema="""Você é um leitor atento de provas respondidas, treinado para capturar não apenas a resposta final, mas os sinais de raciocínio que o aluno deixou — incluindo respostas parciais, incompletas ou em branco.
...
Registre qualquer sinal de que o aluno tentou raciocinar, independentemente do resultado da resposta:
...
- ✅ "Aluno escreveu uma variação do problema; esta etapa apenas descreve o que está visível, sem avaliar a resposta"

Use `null` quando: a resposta está em branco sem rascunho, ou quando não há nenhum sinal observável de raciocínio.

**VOCABULÁRIO PROIBIDO em `raciocinio_parcial`** (qualquer ocorrência destas palavras invalida o JSON inteiro): `correto`, `incorreto`, `errado`, `errada`, `errou`, `acertou`, `deveria ser`, `valor esperado`, `provavelmente`, `possivelmente`, `talvez`, `parece que`, `deve ter`. Use apenas verbos descritivos (escreveu, registrou, deixou, identificou, marcou, riscou) e substantivos neutros (rascunho, sinal, tentativa, fórmula). Para descrever uma resposta que não chegou ao resultado, use "incompleta", "interrompida" ou "sem fechamento", nunca "errada".
  ...
  "respostas": [
    {
      ...
      "raciocinio_parcial": "Descrição neutra de sinais observáveis de raciocínio (rascunho, fórmula, passos). Não use palavras de julgamento como 'errado', 'correto', 'errou'. Use null quando não houver sinais."
    }
  ],

...
def _test_passed(name: str):
    print(f"   [OK] {name}")

def _test_failed(name: str, reason: str):
...
def test_security_validator():
    ...
    for code in safe_codes:
        ...
        if is_safe:
            _test_passed(f"Safe code accepted: {code[:40]}...")
        else:
            all_passed = _test_failed(f"Safe code rejected", f"{violations}")
    ...
    for code, reason in dangerous_codes:
        is_safe, violations = validator.validate(code)
        if not is_safe:
            _test_passed(f"Blocked {reason}: {code[:40]}...")
        else:
            all_passed = _test_failed(f"Should block {reason}", code[:40])
    ...
    for code, should_pass, description in edge_cases:
        ...
        if is_safe == should_pass:
            _test_passed(f"Edge case: {description}")
        else:
            all_passed = _test_failed(f"Edge case: {description}", f"Expected {should_pass}, got {is_safe}")
...
def test_library_detection():
    ...
    for code, expected in test_cases:
        ...
        if not missing:
            _test_passed(f"Detected libraries: {detected}")
        else:
            all_passed = _test_failed(f"Library detection", f"Missing {missing} in {detected}")
...
def test_output_file_detection():
    ...
    for code, expected in test_cases:
        ...
        if not missing:
            _test_passed(f"Detected files: {detected}")
        else:
            all_passed = _test_failed(f"File detection", f"Missing {missing} in {detected}")
...
def test_executor_config():
    ...
    if config.timeout_seconds == 60:
        _test_passed("Default timeout is 60 seconds")
    else:
        all_passed = _test_failed("Default timeout", f"Expected 60, got {config.timeout_seconds}")

    if config.max_memory_mb == 512:
        _test_passed("Default memory is 512 MB")
    else:
        all_passed = _test_failed("Default memory", f"Expected 512, got {config.max_memory_mb}")
    ...
    for lib in required_libs:
        if lib in config.allowed_libraries:
            _test_passed(f"Library allowed: {lib}")
        else:
            all_passed = _test_failed(f"Library not allowed", lib)
...
async def test_e2b_integration():
    ...
    if result.status == ExecutionStatus.SUCCESS and "4" in result.stdout:
        _test_passed("Simple print: 2+2=4")
    else:
        all_passed = _test_failed("Simple print", f"Status: {result.status}, stdout: {result.stdout}")
    ...
    if result.status == ExecutionStatus.SUCCESS:
        _test_passed(f"Pandas import: version {result.stdout.strip()}")
    else:
        all_passed = _test_failed("Pandas import", f"Status: {result.status}, stderr: {result.stderr}")
...
"""
    ...
    if result.status == ExecutionStatus.SUCCESS and len(result.files_generated) > 0:
        ...
        _test_passed(f"Excel generated: {file.filename} ({file.size_bytes} bytes)")
    else:
        all_passed = _test_failed("Excel generation", f"Status: {result.status}, files: {len(result.files_generated)}")
...
"""
    ...
    if result.status == ExecutionStatus.SUCCESS and len(result.files_generated) > 0:
        file = result.files_generated[0]
        _test_passed(f"Chart generated: {file.filename} ({file.size_bytes} bytes)")
    else:
        all_passed = _test_failed("Chart generation", f"Status: {result.status}, files: {len(result.files_generated)}")
    ...
    if result.status == ExecutionStatus.SECURITY_VIOLATION:
        _test_passed("Security violation blocked")
    else:
        all_passed = _test_failed("Security blocking", f"Expected SECURITY_VIOLATION, got {result.status}")
    ...
    if available:
        _test_passed(f"Availability check: {message}")
    else:
        all_passed = _test_failed("Availability check", message)
...
async def test_api_endpoints():
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code == 200:
            data = response.json()
            _test_passed(f"Status endpoint: mode={data['mode']}, available={data['available']}")
        else:
            all_passed = _test_failed("Status endpoint", f"Status code: {response.status_code}")
        ...
        if response.status_code == 200:
            data = response.json()
            _test_passed(f"Libraries endpoint: {len(data['allowed_libraries'])} libraries")
        else:
            all_passed = _test_failed("Libraries endpoint", f"Status code: {response.status_code}")
        ...
        if response.status_code == 200:
            data = response.json()
            if data['is_safe']:
                _test_passed("Validate endpoint: safe code accepted")
            else:
                all_passed = _test_failed("Validate endpoint", f"Safe code rejected: {data['violations']}")
        else:
            all_passed = _test_failed("Validate endpoint", f"Status code: {response.status_code}")
        ...
        if response.status_code == 200:
            data = response.json()
            if not data['is_safe']:
                _test_passed("Validate endpoint: dangerous code blocked")
            else:
                all_passed = _test_failed("Validate endpoint", "Dangerous code not blocked")
        else:
            all_passed = _test_failed("Validate endpoint", f"Status code: {response.status_code}")
        ...
        if response.status_code == 200:
            data = response.json()
            _test_passed(f"Detect endpoint: libs={data['detected_libraries']}, files={data['detected_output_files']}")
        else:
            all_passed = _test_failed("Detect endpoint", f"Status code: {response.status_code}")
        ...
        if response.status_code == 200:
            ...
            if data['status'] == 'success' and '50' in data['stdout']:
                _test_passed(f"Execute endpoint: 10*5={data['stdout'].strip()}")
            else:
                all_passed = _test_failed("Execute endpoint", f"Status: {data['status']}, stdout: {data['stdout']}")
        else:
            all_passed = _test_failed("Execute endpoint", f"Status code: {response.status_code}")
